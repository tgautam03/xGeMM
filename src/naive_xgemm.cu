__global__ void naive_mat_mul_kernel(float* d_A, float* d_B, float* d_C, int N1, int N2, int N3)
{
    // Working on C[i,j]
...
void naive_xgemm(float* d_A, float* d_B, float* d_C, int Nrows_A, int Nrows_B, int Ncols_B, const int dim_block_x, const int dim_block_y)
{
    ...
    naive_mat_mul_kernel<<<dim_grid, dim_block>>>(d_A, d_B, d_C, Nrows_A, Nrows_B, Ncols_B);
}