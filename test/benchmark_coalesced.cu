...
int main(int argc, char const *argv[])
{
    ...
    for (int mat_size = 0; mat_size < 8; mat_size++)
    {
        ...
        coalesced_xgemm(d_A, d_B, d_C_xgemm, Nrows_A, Nrows_B, Ncols_B, 32, 32);
        ...
        matrix_assert(C_cublas, C_xgemm, Nrows_A, Ncols_B);
    ...
    std::cout << "How fast is CuBLAS compared to xGeMM (xGeMM/CuBLAS): ";
    for (int mat_size = 0; mat_size < 8; mat_size++)
