...
#include "../include/tensor_xgemm.cuh"

int main(int argc, char const *argv[])
{
    // Options: 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, etc. (Only multiples of 16!)
    ...
    for (int mat_size = 0; mat_size < n_sizes; mat_size++)
    {
        ...
        for (int i = 0; i < A_FP32.n_rows; i++)
        {
            for (int j = 0; j < A_FP32.n_cols; j++)
                A_FP16.ptr[i*A_FP16.n_cols+j] = (half)A_FP32.ptr[i*A_FP32.n_cols+j];
        }
        for (int i = 0; i < B_FP32.n_rows; i++)
        {
            for (int j = 0; j < B_FP32.n_cols; j++)
                B_FP16.ptr[i*B_FP16.n_cols+j] = (half)B_FP32.ptr[i*B_FP32.n_cols+j];
        }
        ...
        tensor_xgemm(d_A_FP16.ptr, d_B_FP16.ptr, d_C_FP32_xgemm.ptr, d_C_FP32_xgemm.n_rows, d_C_FP32_xgemm.n_cols, d_A_FP16.n_cols);
        cudaDeviceSynchronize();

        // Assert that tensor implementation is correct
    ...
    std::cout << "cuBLAS vs tensor xGeMM (CuBLAS/xGeMM): ";
    for (int mat_size = 0; mat_size < n_sizes; mat_size++)
        std::cout << std::fixed << std::setprecision(2) << cublas_time[mat_size]/xgemm_time[mat_size]*100 << "% ";
    std::cout << "\n";

    // Saving to benchmark file
    update_benckmark_txt("txt_benchmarks/tensor_xgemm.txt", xgemm_time, xgemm_gflops, mat_sizes, n_sizes);
